...
def jam_detector(originalFile, flag):
    ...
    while (video.isOpened()):
        ...
        if frame_counter == 0 or frame_counter % 15 == 0:
            ...
            gray_frame = cv2.GaussianBlur(current_frame_gray, (21, 21), 0) 
            ...
            if previous_frame_gray is not None:
                ...
                percentage.append(movement_checker(cv2.bitwise_and(thresh_frame, thresh_frame, mask=mask), extremes))
        ...
        if frame_counter >= 2 * 15:
            cv2.imshow('frame diff', cv2.bitwise_and(thresh_frame, thresh_frame, mask=mask))  
            #cv2.imshow('frame diff', frame_diff)
            #cv2.imshow('frame diff', cv2.bitwise_and(differ_frame, differ_frame, mask=mask))
