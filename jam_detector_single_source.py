...
def append_to_csv(file_path, mov_persistance, low_percnt, avg, time):
    # Open the CSV file in append mode
    global time_stopped
    date_format = "%YY%mM%dD_%Hh%Mm%Ss"

    if (mov_persistance == 1 and time != 0):
        time_stopped = time.strftime(date_format)
    elif (mov_persistance == 0 and time == 0):
        time_stopped = 0
    current_time = datetime.now()
    ...
    with open(file_path, 'a', newline='') as csvfile:
        ...
        if (mov_persistance > 10):
            date2 = datetime.strptime(time_stopped, date_format)
            time_difference = date1 - date2
            row = [mov_persistance] + [low_percnt] + [avg] + [formatted_time] + [time_difference.total_seconds()]
        else:
            row = [mov_persistance] + [low_percnt] + [avg] + [formatted_time] + [0]
        ...
        if frame_counter == 0 or frame_counter % frame_interval == 0:
            ...
            if previous_frame_gray is not None:
                ...
                if (avg >= percentage[-1] + mov_change_threshold):
                    ...
                    append_to_csv(csv, mov_persistance, percentage[-1], avg, datetime.now())
