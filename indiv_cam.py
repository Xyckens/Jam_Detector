...
import pandas as pd
from csv_file_reader import time_diff, number_detections
            ...
            dbc.Container(
                [
                    ...
                    dbc.Row([
                        dbc.Col([
                            dbc.Nav([
                                dbc.NavItem(dbc.NavLink("Cam 1", href="/")),
...
def cam_table(cams):
    ...
    table = dbc.Table.from_dataframe(df2, striped=True, bordered=True, hover=True, size='sm', style={"width": "fit-content"})
    return table
...
color = ['#0d1366'] * len(cams)
fig = dcc.Graph(
    figure={
        'data': [
            {'x': sources, 'y': detect, 'type': 'bar', 'name': 'SF', 
             'marker': {
               'color': color}},
    ...
    responsive=True,
    style={"width": "fit-content", "height": "70hv"}
)
